...
day_total = sum(
    [infos[0] for infos in activities.values()], datetime.timedelta()
)

att_lines = [
    [0, 0, {
        "action": "sign_in",
        "employee_id": att_def["employee_id"],
        "name": "%s 00:00:00" %(date)
    }],
    [0, 0, {
        "action": "sign_out",
        "employee_id": att_def["employee_id"],
        "name": "%s %s" %(date, day_total)
    }]
]
...
for key, infos in activities.items():
  ...
  # Check if project exists and is timesheetable
    ...
    if len(match_proj) == 0:
        ...
        if category in names:
            ...
            print "Multiple projects found for '%s' in OpenERP.\
             Please fix." %(category)
            sys.exit(1)
    else:
  ...
  # Find analytical account matching project
    ...
    if tag is None:
        ...
        if len(match_task) == 0:
            print "No task found for '%s' in account '%s'\
             in OpenERP. Please fix." %(tag, category)
            sys.exit(1)
        elif len(match_task) > 1:
            print "Multiple tasks found for '%s' in account '%s'\
             in OpenERP. Please fix." %(tag, category)
            sys.exit(1)
        else:
  ...
  #act_id, act_name = tiny.search_activity(acc_id, tiny_activity)[0]
    ...
    amount = tiny.on_change_unit_amount(
        ts_def["product_id"],
        duration_hours,
        ts_def["product_uom_id"]
    )["value"]["amount"]
