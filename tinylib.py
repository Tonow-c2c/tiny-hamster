...
class TinyXMLRPC:
    ...
    def _connect(self):
        ...
        self._common_server = ServerProxy(os.path.join(
            self.base_url, "common"
        ), context=ssl_context)
        self._object_server = ServerProxy(os.path.join(
            self.base_url, "object"
        ), context=ssl_context)
        self._wizard_server = ServerProxy(os.path.join(
            self.base_url, "wizard"
        ), context=ssl_context)

    def _login(self):
        self.user_id = self._common_server.login(
            self.database,
            self.username,
            self.password
        )
    ...
    def execute(self, *params):
        return self._object_server.execute(
            self.database,
            self.user_id,
            self.password,
            *params
    ...
    def wizard(self, *params):
        return self._wizard_server.execute(
            self.database,
            self.user_id,
            self.password,
            *params
  ...
  #  each account is a list [ account_id, account_name ]
    ...
    def timesheet_write(self, ts_id, ts_lines, att_lines=[]):
        return self.execute(
            ...
            "write",
            [ts_id],
            {"timesheet_ids": ts_lines, "attendances_ids": att_lines},
            {"lang": "fr_FR",
             "tz": "Europe/Paris",
             "active_ids": [],
             "read_delta": 0.0,
